// @surface-orphan-ok — kept deliberately ahead of its caller: the Object
// Explorer and dashboard map view are the next consumers, and rebuilding the
// PMTiles/maplibre plumbing would cost far more than parking it.
//
// Object map — a pin per object with a geopoint property, positioned by lat/lng,
// sized by an optional weight, click to select. maplibre
// is dynamically imported so it's code-split out of the main bundle.
...
/** One plottable object: anything in the ontology with a geopoint property.
 *  `weight` sizes the pin and `urgent` picks the one warm colour, so the map
 *  stays a renderer and the caller decides what "needs a person" means. */
...
const weightOf = (p: MapPoint) => p.weight ?? 0
function pinColor(p: MapPoint): string {
  if (p.urgent === true)  return APPROVALS
  if (weightOf(p) > 0)    return OPEN_WORK
  return CLEAR
}
...
function buildMarker(h: MapPoint, onClick: () => void): HTMLElement {
  const load = weightOf(h)
  const size = 16 + Math.min(14, load * 2)
  const wrap = document.createElement('div')
  wrap.style.cssText = 'display:flex;flex-direction:column;align-items:center;cursor:pointer;'
  wrap.title = load > 0 ? `${h.label}: ${String(load)}` : h.label
...
export function ObjectMap({ points: hotels, onSelect: onHop, title }: { points: MapPoint[]; onSelect: (id: string) => void; title?: string }) {
  ...
  const locatedKey = located.map((h) => `${h.id}:${String(h.lat)}:${String(h.lng)}`).join('|')
  ...
  useEffect(() => {
    ...
    void import('maplibre-gl').then(async ({ default: maplibregl }) => {
      ...
      for (const h of located) {
        ...
        markers.push(
          new maplibregl.Marker({ element: buildMarker(h, () => { onHopRef.current(h.id) }), anchor: 'center' })
...
function SvgPinMap({ hotels, onHop }: { hotels: MapPoint[]; onHop: (id: string) => void }) {
  ...
  return (
    <svg viewBox={`0 0 ${String(W)} ${String(H)}`} preserveAspectRatio="xMidYMid meet" className="absolute inset-0 h-full w-full">
      ...
      {hotels.map((h) => {
        ...
        return (
          <g key={h.id} transform={`translate(${String(x)},${String(y)})`} className="cursor-pointer" onClick={() => { onHop(h.id) }}>
            ...
            <text textAnchor="middle" y={r + 14} fontSize={12} fontWeight={600} fill="#C5CBD3">{h.label}</text>
          </g>
        )
      })}
    </svg>
