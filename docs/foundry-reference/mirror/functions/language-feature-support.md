<!-- source: https://palantir.com/docs/foundry/functions/language-feature-support/ · mirrored 2026-08-18 from Palantir Foundry docs -->
...
| Media handling support           | No        | Yes           | Partial       | Partial | The ability to interact with [media items](/docs/foundry/functions/media/). TypeScript v1 supports full media operations, including document OCR and audio transcription. TypeScript v2 and Python support media uploads via Ontology edits and basic operations on media properties, but do not support document-specific or audio-specific operations. |
...
| Functions on models support      | Yes       | Yes           | Yes           | Yes    | Executing live deployment models [from a function](/docs/foundry/functions/functions-on-models/). TypeScript v2 and Python require ontology-bound model functions.   |
| Language model integration       | No        | No            | Yes           | Yes    | Calling language models through [proxy endpoints](/docs/foundry/functions/language-models-python-tsv2/) using model aliases and Platform SDK imports.                 |
...
| Staged writes support \[Beta]     | No        | No            | Yes           | No     | The ability to use [staged writes](/docs/foundry/functions/typescript-v2-staged-writes/) for read-after-write guarantees and atomic execution in edit functions.      |
...
* **Staged writes \[Beta]:** TypeScript v2 supports [staged writes](/docs/foundry/functions/typescript-v2-staged-writes/), an alternative execution model for Ontology edit functions that provides read-after-write guarantees within the function and allows nested calls to other staged-write functions.
