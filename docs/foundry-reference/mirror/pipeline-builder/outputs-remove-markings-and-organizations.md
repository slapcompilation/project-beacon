<!-- source: https://palantir.com/docs/foundry/pipeline-builder/outputs-remove-markings-and-organizations/ · mirrored 2026-08-18 from Palantir Foundry docs -->
...
![The Settings and Manage branches options in Pipeline Builder.](./images/markings-settings.png)

2. Select the **Branch protection** tab.

![The Manage branches branch protection pop-up menu in Pipeline Builder.](./images/markings-manage-branch.png)
...
![An example approval policy in Pipeline Builder.](./images/markings-example-code-approval.png)
...
![Security approvals tab in Pipeline Builder.](./images/markings-security-approvals.png)
...
![The Pipeline outputs tab in Pipeline Builder.](./images/markings-pipeline-output.png)

3. Select the **Configure markings** dropdown menu under the output dataset.

![Configure markings dropdown on Pipeline Builder output.](./images/markings-configure-markings-option.png)

4. To remove markings: Under the **Markings** tab in the pop-up menu, select the red remove icon next to the marking(s) you want to remove. You can search for markings by name or by marking ID. To search by ID, select the configuration icon in the **Security markings** modal and choose **Search by ID**, then enter the marking ID in the **Search all markings IDs** search bar.

![A pop-up dialog for removing markings on an output in Pipeline Builder.](./images/markings-inherited-markings.png)

The removed markings will now show up under the **Markings removed** section in the dialog.

![The removed markings in the "Markings removed" section in the pop-up dialog.](./images/markings-removed.png)
...
![Organization unmarkings tab in unmarkings workflow.](./images/markings-organizations.png)
...
![The "Remove marking" icon on an output in Pipeline Builder.](./images/markings-removed-pop-up.png)
...
![The warning icon denoting that the organizations for this input have been modified since the previous removal action.](./images/markings-organization-warnings.png)
...
![A proposal to remove a marking from a pipeline output.](./images/markings-approval-page-pull-request.png)
...
![Configure markings dropdown menu on Pipeline Builder output.](./images/markings-configure-markings-option.png)

4. For markings: Select the undo icon next to the **Markings not propagated** section in the pop-up dialog.

![The marking removal pop-up dialog, with the option to undo a marking that stopped propagating.](./images/markings-undo-remove-marking.png)

5. For organizations, select the undo icon next to the inputs associated with the organization.

![The marking removal pop-up dialog, with the option to undo an organization that stopped propagating.](./images/markings-organization-undo.png)

6. Select **Apply**, then save your pipeline from the top right of your screen.

![The "Save" option for the pipeline, found in the top right of the screen.](./images/markings-save-pipeline.png)
...
In a job group, markings from all inputs will be inherited by all outputs within the same job group. Pipeline Builder displays a badge icon with a tooltip on outputs in a job group when those outputs may have markings added by other inputs in the same job grouping. This visual indicator helps you identify when markings are being inherited across your pipeline components. To view an example and learn more about job groups, review our [documentation](/docs/foundry/pipeline-builder/management-job-groups/).
