<!-- source: https://palantir.com/docs/foundry/pipeline-builder/datasets-overview/ · mirrored 2026-09-04 from Palantir Foundry docs -->
...
**Semi-structured data** refers to a **dataset** that consists of files without a schema, making the data non-tabular. Thus, semi-structured datasets are sometimes called "schema-less" datasets. Pipeline Builder supports semi-structured data in the form of CSV, JSON, GeoJSON, XML, Microsoft Excel, KML, shapefile, plain text, and email files. You can use parsing transform functions to convert semi-structured files into tabular form and benefit from schema safety. For example, the [Extract rows from an Excel file](/docs/foundry/pb-functions-transform/parseExcelV2/) transform reads `.xls`, `.xlt`, `.xltm`, `.xltx`, `.xlsx`, and `.xlsm` files. For XML data, Pipeline Builder can auto-generate schemas from XSD files—upload your XSD files to a raw dataset and reference them inside a Parse XML transform to automatically create the appropriate schema structure. Learn how to [transform data in your pipeline](/docs/foundry/pipeline-builder/transforms-transform-data/).
