...
![action\_wizard](./images/action_wizard.png)
...
![save\_action](./images/save_action.png)
...
![on\_click\_2](./images/on_click_2.png)
...
![modify flight](./images/modify_flight.png)
...
![action submit](./images/action_submit.png)
...
![action complete](./images/action_complete.png)

#### Chaining an event with an action

To run a Workshop event at a specific point in an action's lifecycle, select one of these event triggers at the bottom of the widget configuration:

* **On start of Action submission:** Runs the event when the action submission begins.
* **On successful completion of Action submission:** Runs the event after the action submission succeeds.

Use these triggers to refresh data, navigate to another page, or update variables during an action workflow.
