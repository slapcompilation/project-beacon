<!-- source: https://palantir.com/docs/foundry/quiver/cards-index-writeback/ · mirrored 2026-09-04 from Palantir Foundry docs -->

# Writeback cards

Back to: [Index of cards](/docs/foundry/quiver/cards-index/)

**Action buttons** can be added to your Quiver analysis canvas, time series charts, and dashboards to allow users to write data back to the Ontology. Example writeback actions include creating objects, updating properties on existing objects, or modifying object links.
...
**Example: Create an `Annotation` object instance**
...
![The search bar showing an action button card option after searching for the word 'action'](./images/howto-action-button-create-annotation-example.png)

**Example: Adjust operational values**

You can also invoke an Ontology action to update a property value based on an observed phenomenon. For example, the time series chart below displays two curves that show the predicted total cost per barrel for two types of products, Gasoline (in blue) and Diesel (in red). Today’s actual total cost per barrel (y-axis) for Gasoline (dark blue dots) ranges from 6.35 to 8.45 cents/barrel. This is far from the minimal predicted total cost of 6.01 cents/barrel. To lower the production cost, you can adjust the DRA concentration rate from 15 ppm to a value closer to 6.5 ppm. Select a range around 6 on the x-axis, then select the **Adjust DRA level** Ontology action, which is automatically set with the selection boundary values.

![The search bar showing an action button card option after searching for the word 'action'](./images/howto-action-button-adjust-values-example.png)
...
![The search bar showing an action button card option after searching for the word "action"](./images/howto-action-button-add.png)
...
3. Select an existing Ontology [action](/docs/foundry/action-types/overview/) from the dropdown menu. If the action does not exist yet, you will need to [create a new action](/docs/foundry/action-types/getting-started/) in the Ontology.

<img alt="The action button editor showing the select action dropdown menu with existing actions to select from." src="./images/howto-action-button-select-action.png" width="350px">
...
<img alt="Action button editor showing default input configuration" src="./images/action-default-input.png" width="450px">
...
<img alt="Action button form submission showing the pre-filled input" src="./images/action-default-input-form.png" width="450px">
...
2. Under the **Action Buttons** section, open the dropdown menu and select an existing action button from the analysis or create a new one. The Ontology action button will now be visible in the selection menu.

<img alt="The time series chart editor showing the select Ontology action button dropdown menu with existing buttons to select from." src="./images/howto-action-button-add-to-time-series-chart.png" width="350px">

## Use selection boundary values as input to Ontology actions

When an Ontology action button is added to a time series chart, you can parameterize the action inputs using the boundary values of the x- or y-axis selection.
...
<img alt="An action button showing a time series chart override." src="./images/action-time-series-override.png" width="350px">
...
<img alt="Action button form submission showing the pre-filled input form chart selection" src="./images/action-time-series-override-form.png" width="450px">
...
<img alt="The action button editor window showing the button display configuration options." src="./images/howto-action-button-display-config.png" width="350px">
