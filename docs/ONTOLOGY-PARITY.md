...
- **Per-datasource conflict resolution strategy (apply user edits / apply most recent value)** — **BUILT 2026-09-22 (842), and the census had it backwards.** This was filed as an engine nothing reaches. Measured, it was the opposite: `conflict_resolution`, `timestamp_property_id`, the CHECK and `guard_conflict_resolution` all existed, and `object_state` never read any of them — so a datasource set to `apply_most_recent_value` behaved exactly like the default. A settable value that changes nothing is worse than an absent one. 842 builds the second strategy per property, the create path's own timestamp diamond (which only the flowchart states), the edit-only escape and the switch timestamp that pre-existing edits are measured against, and renders the control on the Datasources tab. *(mirror/object-edits/how-edits-applied.md)*
