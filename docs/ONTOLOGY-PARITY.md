...
- **Funnel pipeline graph and hydration status in the Datasources tab (OSv2 node green tick, Failed job link, Stream bubble)** — The graph's exact shape is evidenced only by screenshots (pipeline1.png, pipeline2.png, pipeline3.jpg, faq_indexing.png) which I did not open, per this run's no-images rule. *(mirror/object-indexing/funnel-batch-pipelines.md)*
- **Monitor the health of an object type: Funnel job monitoring rules + Sync Propagation Delay** — The 'Monitor the health of this object type' entry point from Ontology Manager into monitoring views has no rule family to land on. *(mirror/object-indexing/funnel-batch-pipelines.md)*
