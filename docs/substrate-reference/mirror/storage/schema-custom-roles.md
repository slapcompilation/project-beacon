<!-- source: https://supabase.com/docs/guides/storage/schema/custom-roles · mirrored 2026-08-13 from Supabase docs -->
...
Create a custom role `manager` to provide full read access to a specific bucket. For a more advanced setup, see the [RBAC Guide](https://supabase.com/docs/guides/api/custom-claims-and-role-based-access-control-rbac#create-auth-hook-to-apply-user-role).
...
Danger: Signing a new JWT requires your `JWT_SECRET`. You must store this secret securely. Never expose it in frontend code, and do not check it into version control.
