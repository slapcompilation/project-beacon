<!-- source: https://supabase.com/docs/guides/platform/temporary-access · mirrored 2026-08-13 from Supabase docs -->
...
Note: Projects need to be at least on Postgres 17.6.1.081 (or higher) to enable temporary access. You can find the Postgres version of your project on the [General Settings](https://supabase.com/dashboard/project/_/settings/general) page. If your project is on an older version, you will need to [upgrade](https://supabase.com/docs/guides/platform/upgrading) to use this feature.

## Manage temporary access via the dashboard

The easiest way to manage temporary access is via the "Enable temporary access" settings section in [Database Settings page](https://supabase.com/dashboard/project/_/database/settings) of the dashboard.
...
Note: Postgres roles can still be accessed using the password configured on the role. This allows long-lived service connections to continue using the existing credentials.
...
Note: This feature does not work with IPv6 Transaction pooler (PgBouncer). Direct connections and connections through the IPv4 connection pooler are fully supported.
