<!-- source: https://supabase.com/docs/guides/auth/social-login/auth-kakao · mirrored 2026-08-13 from Supabase docs -->
...
Note: In the Kakao Developers Portal, the "account\_email" consent item is only available for apps that are registered as "Biz App". To convert your app to a "Biz App", go to **App Settings** > **App** > **General**, and complete the required fields in the **Business Information** section.
...
When your user signs in, call [`signInWithOAuth()`](https://supabase.com/docs/reference/dart/auth-signinwithoauth) with `kakao` as the `provider`:
...
When your user signs in, call [signInWith(Provider)](https://supabase.com/docs/reference/kotlin/auth-signinwithoauth) with `Kakao` as the `Provider`:
...
When your user signs in, call [`SignIn()`](https://supabase.com/docs/reference/csharp/sign-in-with-oauth) with `Provider.Kakao` as the provider:
