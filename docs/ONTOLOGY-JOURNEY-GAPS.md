...
| **The Check access panel for an object type** | ~~`check_access` RAISES on `object_type`~~ — **shipped, 834.** Both published sections are filled: Access requirements (organizations — a **set** here, since an object type has no `organization_id` and `auth_in_ontology` admits any of the ontology space's — plus markings and the roles clause) and Additional data requirements (the datasources, per *"a Workshop module that displays a table of objects requires access to the object type & its datasources"*). The data section reports **markings**, each decided by `marking_member(m, p_user)`, because 833's `datasource_readable` answers for the **caller** through `auth.uid()` while this function answers for `p_user` — calling it would have reported the administrator's own access under someone else's name, and looked right in every screenshot taken by someone who has access. It follows 833's either/or: a configured policy replaces the datasource requirement with the policy's own markings. A policy's **granular arm is deliberately not a row** — it is a predicate over a row's values, so it has no per-user answer. `CheckAccessPanel` was mounted only on `DatasetPage`; it is now on the object type's Security tab |
...
| `check_access` on an object type | settled three ways: the panel works on "a Project, folder, or file", an object type **is** a file, and `ontology-in-project.png` shows it as a peer row of a dataset. **Built, 834** — and a fourth sentence settled the *contents*, which the three above did not: the object security page names object types as something the panel reaches "including access to dependent object types, their data sources, and granular access controls" |
