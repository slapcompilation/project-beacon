...
| A6 | **Flywheel observability** — ECE trend, ontology-growth, copilot rule-citation events + a dashboard | *(shipped 2026-07)* Flywheel page (existed: calibration/loop/reliability) extended with the Learning-loop card (rules taught, vocabulary grown, decisions shaped via influenced_by edges, approval lift with-vs-without rules) + monthly ECE trend; e2e-gated |
| A7 | **Behavior evals** — calibration-veto agent eval; rubric that the copilot refuses a violating ask | *(shipped 2026-07)* both named suites existed (guardrail 7 cases, calibration-veto 4) — CI-run, per-case persisted; gap closed: the #336 batch guard extracted to evaluateBatchApprovals + 3 behavior cases. **Backlog complete.** |
