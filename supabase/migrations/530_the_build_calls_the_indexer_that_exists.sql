-- Finishing 529's revert: the build calls the one-argument indexer again.
--
-- 529 restored index_object_type(uuid) and dropped the two-argument form, and
-- left run_build_job still calling the two-argument one — so every index job
-- failed and five cases across indexBuild and objectDatasets went red.
--
-- A revert is a change like any other, and I shipped half of it. The suite
-- caught it on the next run, which is the third time in this arc that the
-- standing tests have been the thing that noticed.
...
BEGIN
  IF pg_get_functiondef('public.run_build_job(uuid)'::regprocedure) LIKE '%output_object_type_id, p_job%' THEN
    RAISE EXCEPTION 'run_build_job still calls an indexer that no longer exists';
  END IF;
  -- Executed: an index job must actually run end to end.
  SELECT bj.output_object_type_id INTO t FROM public.build_jobs bj
   WHERE bj.output_object_type_id IS NOT NULL LIMIT 1;
  IF t IS NOT NULL THEN
    PERFORM set_config('request.jwt.claims',
      (SELECT json_build_object('sub', u.id::text, 'app_metadata',
         json_build_object('role', u.role, 'org_id', u.organization_id))::text
         FROM public.users u WHERE u.role IN ('owner','admin') LIMIT 1), true);
    b := public.run_index_build(ARRAY[t], true);
    IF b IS NULL THEN RAISE EXCEPTION 'the backfill path stopped working'; END IF;
    PERFORM set_config('request.jwt.claims', '', true);
  END IF;
  RAISE NOTICE '530: the build calls the indexer that exists';
END $$;
