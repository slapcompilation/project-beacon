-- Migration 078: Update briefing action_url values to workspace paths
-- All old standalone routes now redirect to workspace panels.
-- This migration patches get_briefing_actions() so deep-links from the
-- Briefing page navigate directly to the correct workspace tab without a
-- double-redirect hop.
--
-- URL mapping:
--   /restocks                → /flow?panel=approvals
--   /procurement             → /mind?panel=procurement
--   /procurement?tab=saved   → /mind?panel=procurement
--   /invoicing               → /mind?panel=intelligence
--   /negotiation-prep        → /mind?panel=intelligence
--   /flow-dashboard          → /flow?panel=timeline
--   /expiry                  → /floor?panel=expiry
--   /gl-export               → /mind?panel=gl
...
LANGUAGE sql STABLE AS $$
  ...
  -- ══ I: Autonomous restock proposals ══════════════════════════════════════════
  SELECT
    ...
    '/mind?panel=procurement'::text, 'Approve & Create PO'::text,
    jsonb_build_object(
  ...
  SELECT
    ...
    '/flow?panel=approvals'::text, 'Request restock'::text,
    jsonb_build_object(
  ...
  SELECT
    ...
    '/mind?panel=intelligence'::text, 'Review discrepancy'::text,
    jsonb_build_object(
  ...
  SELECT
    ...
    '/flow?panel=timeline'::text, 'View write-offs'::text,
  ...
  SELECT
    ...
    '/floor?panel=expiry'::text, 'View expiry'::text,
  ...
  SELECT
    ...
    '/mind?panel=procurement'::text, 'View order'::text,
  ...
  SELECT
    ...
    gl.unmapped_count::text || ' transaction'
      ...
      || CASE WHEN EXTRACT(DAY FROM CURRENT_DATE) >= 25
              ...
              ELSE '' END,
    '/mind?panel=gl'::text, 'Map accounts'::text,
  ...
  SELECT
    ...
    '/mind?panel=gl'::text, 'Export now'::text,
