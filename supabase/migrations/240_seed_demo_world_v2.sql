-- Recovered from supabase_migrations version 20260607085112.
...
BEGIN
  ...
  FOR rec IN SELECT * FROM _cat LOOP
    ...
    VALUES (p_hotel_id, rec.name, rec.sku, rec.cost, true) RETURNING id INTO rec.product_id;
    INSERT INTO product_variants (
      product_id, name, sku, current_stock, cost, low_stock_threshold,
      enabled, unit_of_measure, par_source, default_supplier_id
    ) VALUES (
      rec.product_id, 'Standard', rec.sku || '-STD', rec.par * 2, rec.cost, rec.par,
      true, rec.unit, 'auto', CASE WHEN rec.sup = 'A' THEN v_supplier_a ELSE v_supplier_b END
  ...
  FOR rec IN SELECT * FROM _menu LOOP
    ...
    VALUES (p_hotel_id, rec.name, rec.category, rec.price, true) RETURNING id INTO rec.menu_id;
  ...
  FOR rec IN SELECT * FROM _cat LOOP
    DECLARE
      ...
      v_day date; v_dow int; v_demand int; v_lift numeric; v_ts timestamptz;
      v_pending_qty int := 0; v_pending_day date := NULL;
    BEGIN
      FOR v_day IN
        SELECT d::date FROM generate_series((now() - (p_days || ' days')::interval)::date, now()::date, interval '1 day') AS d
      LOOP
  ...
  JOIN stock_logs sl ON sl.variant_id = mii.variant_id AND sl.source = 'pos'
    AND sl.timestamp::date = ps.sale_time::date AND sl.hotel_id = p_hotel_id
  ...
  FOR rec IN SELECT id, name, coalesce(lead_time_days,7) AS lead FROM suppliers
    ...
    BEGIN
      FOR i IN 1..4 LOOP
        ...
        VALUES (
          p_hotel_id, v_po, 'INV-DEMO-' || to_char(v_ts,'YYYYMMDD') || '-' || i::text,
          (v_ts + (rec.lead || ' days')::interval)::date,
          v_amt + v_disc * (CASE WHEN random() > 0.5 THEN 1 ELSE -1 END), v_amt, v_disc, 'approved', v_actor
        );
      END LOOP;
    END;
  END LOOP;
  ...
  RETURN jsonb_build_object(
    'hotel_id', p_hotel_id, 'days', p_days, 'variants_made', v_variants_made,
